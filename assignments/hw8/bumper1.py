...
def main():
    width = randint(200, 350)
    height = width + randint(0, 175)
    win = GraphWin("Bumper", width, height)
    win.setBackground(get_random_color())

    circ1 = Circle(Point(randint(30, int(width) / 2 - 30),
                         randint(30, int(height) - 30)), 25)
    circ1_color = get_random_color()
    circ1.setFill(circ1_color)
    circ1.draw(win)

    circ2 = Circle(Point(randint(int(width)/2 + 30, int(width) - 30),
                         randint(30, int(height) - 30)), 25)
    circ2_color = get_random_color()
    circ2.setFill(circ2_color)
    circ2.draw(win)

    circ1_x = get_random(5)
    circ1_y = get_random(5)
    circ2_x = get_random(5)
    circ2_y = get_random(5)

    while win.isOpen():
        if did_collide(circ1, circ2):
            distance_y = abs(circ1.getCenter().getY() - circ2.getCenter().getY())
            distance_x = abs(circ1.getCenter().getX() - circ2.getCenter().getX())
